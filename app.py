...
from create_necessary_outputs import create_necessary_outputs_function, generate_playlist_feature
...
SPOTIPY_CLIENT_ID = '8e5af1ce0cdd49bf88f0898e3a3de3eb'
SPOTIPY_CLIENT_SECRET = 'a293ae9a1f2d4991b5214759bc2f8824'
...
def recommendations():
    ...
    selected_playlist_name = [name for name, id in id_name.items() if id == selected_playlist_id][0]
    ...
    data_w_genre = filter_artists(pd.read_csv('artists.csv'), target_genres=['classical', 'jazz', 'reggae', 'rock', 'pop', 'electronics', 'hip_hop', 'hip hop', 'rap'])
    ...
    playlist_EDM = create_necessary_outputs_function(selected_playlist_name, id_name, spotify_df, sp)
    ...
    edm_top40 = generate_playlist_recos(spotify_df, complete_feature_set_playlist_vector_EDM, complete_feature_set_nonplaylist_EDM, sp)
    ...
    return render_template("index.html", playlists=id_name, recommendations=recommendations)

def get_playlists(sp):
    playlists = sp.current_user_playlists()
    id_name = {item['name']: item['id'] for item in playlists['items']}
    return id_name
